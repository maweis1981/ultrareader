...
export function CompletionScreen({
  ...
  // Determine video format from mime type
  const isMP4 = recordedMimeType?.startsWith('video/mp4') ?? false;
  ...
  const handleDownloadVideo = () => {
    ...
    a.href = videoUrl;
    a.download = `ultrareader-${article?.title || 'reading'}-${Date.now()}.${videoExt}`;
  ...
  const handleShareVideo = async () => {
    ...
    if (navigator.share && navigator.canShare) {
      const file = new File([recordedBlob], `ultrareader-reading.${videoExt}`, { type: mimeType });
    ...
    <>
      ...
      <div className="completion-screen">
        <div className="completion-content">
          ...
          {/* Format info for WebM */}
          {recordedBlob && !isMP4 && (
            <p className="video-format-note">
              Video format: WebM (best played in Chrome/Firefox)
            </p>
          )}
          ...
          <div className="completion-actions">
            {recordedBlob && (
              <button className="completion-btn share" onClick={handleShareVideo}>
                ...
                Share Video ({videoExt.toUpperCase()})
              </button>
            )}
